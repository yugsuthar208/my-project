...
import { getTripCoverImage, handleImageError } from "../../utils/imageResolver";
...
export function TripCard({ trip, index = 0 }) {
  ...
  const thumbUrl = getTripCoverImage(trip, index);
  ...
  return (
    <Link to={`/trips/${trip.id}`} style={{ display: "block" }} className="stagger-item">
      <div className="card card--hover" style={{ display: "flex", overflow: "hidden", cursor: "pointer", height: "100%" }}>
        ...
        <div style={{ width: 120, flexShrink: 0, background: "#18181b", position: "relative", overflow: "hidden" }}>
          <img
            src={thumbUrl}
