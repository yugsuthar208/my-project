...
import { getCityCoverImage, handleImageError } from "../utils/imageResolver";

function CityCard({ city, index = 0 }) {
  const imgUrl = getCityCoverImage(city, index);
  const cityName = city.name || city.city_name || "Exotic Destination";
  const cost = city.cost_index ? Math.round(city.cost_index * 40) : (city.estimated_trip_cost ? Math.round(city.estimated_trip_cost) : 2500);
  ...
  return (
    ...
    >
      <div style={{ position: "relative", width: "100%", height: 160, background: "#18181b", overflow: "hidden" }}>
        <img
          src={imgUrl}
          alt={cityName}
        ...
        <div style={{
          ...
          ★ {city.popularity_score ? Number(city.popularity_score).toFixed(1) : "9.5"}
        </div>
      </div>
      <div style={{ padding: "16px" }}>
        <p style={{ fontWeight: 700, fontSize: "1.1rem", marginBottom: "4px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {cityName}
        </p>
        <p style={{ color: "var(--ink-soft)", fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "4px" }}>
          <MapPin size={12} /> {city.country || "Global"}
        </p>
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontWeight: 600, color: "var(--ink)", fontSize: "0.9rem" }}>
            ₹{cost.toLocaleString('en-IN')}/night
          </span>
      ...
      <motion.section variants={itemVariants}>
        ...
        {citiesLoading ? (
          ...
          <div style={{ 
            ...
            {cities.map((city, idx) => (
              <div key={city.id || city.city_id || idx} style={{ scrollSnapAlign: "start" }}>
