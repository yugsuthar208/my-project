...
import { ArrowLeft, Settings, Plus, LayoutList, Map as MapIcon, Calendar, Info, Share2, Users, Compass, Utensils, Train, MapPin, Maximize2, Minimize2 } from "lucide-react";
...
export default function TripDetailPage() {
  ...
  const [panelWidthMode, setPanelWidthMode] = useState("wide"); // "wide" (52%) or "expanded" (65%)
  
  // Modals
  const [showAddStop, setShowAddStop] = useState(false);
  const [pickerStop, setPickerStop] = useState(null);
  ...
  const loadTrip = useCallback(() => {
    tripService.getTrip(id)
      .then(res => {
        ...
        const budgetObj = rawData?.budget || rawData?.budget_summary || rawData?.trip?.budget || rawData?.trip?.budget_summary;
        ...
        if (tripData?.transit_legs?.length > 0) {
          setSelectedLegId(prev => (prev && tripData.transit_legs.some(l => l.id === prev) ? prev : tripData.transit_legs[0].id));
        }
      })
  ...
  const handleAddActivity = async (activity, targetStopId) => {
    try {
      ...
      addToast({ message: `"${activity.name}" added to itinerary!` });
      // Keep modal open so user can add multiple activities without reopening
      loadTrip();
    } catch(e) {
  ...
  const handleSelectTransitOption = async (legId, optionId, optionMode) => {
    // 1. Instant optimistic state update for real-time map polyline switch
    setTrip(prev => {
      if (!prev) return prev;
      const updatedLegs = (prev.transit_legs || []).map(l => {
        if (l.id === legId) {
          const selectedOpt = optionId 
            ? (l.options?.find(o => o.id === optionId) || { id: optionId, mode: optionMode })
            : null;
          return {
            ...l,
    ...
    // 2. Persist to backend and refresh budget
    try {
      await tripService.selectTransitOption(id, legId, optionId);
      addToast({
        message: optionId 
          ? `Switched route to ${(optionMode || 'new mode').toUpperCase()}!` 
          : "Transport option unselected for this leg."
      });
      loadTrip();
    } catch (err) {
      addToast({ message: err.message || "Failed to update transit option", type: "error" });
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{
          width: panelWidthMode === "expanded" ? "min(960px, 65vw)" : (activeTab === "budget" || activeTab === "transit" || activeTab === "discover" ? "min(780px, 52vw)" : "min(680px, 48vw)"),
          maxWidth: "100%",
          ...
          <div style={{
            ...
            {/* Panel Expand / Standard Toggle */}
            <div style={{ paddingBottom: 8, paddingLeft: 8, display: "flex", alignItems: "center" }}>
              <button
                onClick={() => setPanelWidthMode(prev => prev === "expanded" ? "wide" : "expanded")}
                ...
                style={{
                  ...
                  color: panelWidthMode === "expanded" ? "var(--ink)" : "var(--ink-soft)",
                  background: panelWidthMode === "expanded" ? "rgba(15, 23, 42, 0.08)" : "transparent",
                }}
                title={panelWidthMode === "expanded" ? "Standard side panel (52%)" : "Expand side panel (65%)"}
              >
                {panelWidthMode === "expanded" ? <Minimize2 size={15} /> : <Maximize2 size={15} />}
              </button>
            </div>
          </div>

          <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", maxWidth: "100%", boxSizing: "border-box", padding: "24px 24px" }}>
            {activeTab === "itinerary" && (
              <>
            ...
            {activeTab === "budget" && <TripBudget trip={trip} tripId={trip.id} budget={trip.budget} onRefresh={loadTrip} />}
      ...
      {pickerStop && (
        <ActivityPickerModal
          tripId={trip.id}
          stopId={pickerStop.id}
          stopName={pickerStop.city?.name || pickerStop.city_name || "Destination"}
          cityId={pickerStop.city?.id || pickerStop.city_id}
          existingItemIds={(pickerStop.itinerary_items || pickerStop.stop_activities || selectedStopActivities || []).map(a => a.activity_id || a.activity?.id)}
