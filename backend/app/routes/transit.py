            ...
            "options": [
                {
                    ...
                    "departure_time": (opt.metadata_json or {}).get("departure_time"),
                    "arrival_time": (opt.metadata_json or {}).get("arrival_time"),
                    "service_number": (opt.metadata_json or {}).get("service_number"),
                    "operating_days": (opt.metadata_json or {}).get("operating_days"),
                    "class_type": (opt.metadata_json or {}).get("class_type"),
                    "baggage": (opt.metadata_json or {}).get("baggage"),
                    "amenities": (opt.metadata_json or {}).get("amenities", []),
                    "route_via": (opt.metadata_json or {}).get("route_via"),
...
@router.patch(
    ...
    status_code=status.HTTP_200_OK,
    summary="Select or unselect a transit option for a specific leg",
)
async def select_transit_option(
    ...
    """
    Selects or unselects a transit option for a leg and updates the authoritative backend state.
    """
    ...
    # Handle unselect / clear option
    if not payload.selected_option_id:
        leg.selected_option_id = None
        leg.selected_option = None
        db.add(leg)
        await db.flush()
        await db.commit()
        ...
        return APIResponse(
            ...
            message="Transit option unselected successfully. Budget recalculated."
        )
